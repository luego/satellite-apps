...
export function TimerDisplay({ snapshot, stateLabel, primaryLabel }: TimerDisplayProps) {
  ...
  const availableTimerHeight = height - (isCompactLandscape ? 180 : 128);
  const ringSize = Math.max(
    isCompactLandscape ? 160 : 170,
    Math.min(width * (isCompactLandscape ? 0.3 : 0.42), availableTimerHeight, isCompactLandscape ? 250 : 340),
  );
  const maxTimerSize = isCompactLandscape
    ? 76
    : width < 680
      ? typography.timerDisplayMobile.fontSize
      : typography.timerDisplay.fontSize;
  ...
  return (
    <View style={styles.container}>
      <Text adjustsFontSizeToFit numberOfLines={1} style={[styles.state, { color: stateColor }]}>
        {stateLabel}
      </Text>
      <View
        style={[
      ...
      <Text adjustsFontSizeToFit numberOfLines={1} style={[styles.label, isOvertime ? { color: stateColor } : null]}>
